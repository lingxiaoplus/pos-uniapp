<template>
	...
	<mu-paper :z-depth="0" class="demo-loadmore-wrap">
		<mu-appbar style="width: 100%;position: fixed;top: 0;" color="primary">
			<mu-button icon slot="left" @click="back"><mu-icon value="arrow_back"></mu-icon></mu-button>
			<p>注册账号</p>
		</mu-appbar>
	<view class="register">
		...
		<view class="content">
			...
			></wButton> -->
			<mu-flex justify-content="center" align-items="center" style="margin-left: 80rpx;margin-right: 80rpx;">
				<mu-button v-loading="isRotate" full-width round large color="primary" @click="startReg()">
					注 册
				</mu-button>
			</mu-flex>
			<!-- 底部信息 -->
	...
	export default {
		...
		methods: {
			...
			async getVerCode(){
				...
				try{
					let resp = await this.$request.post("/user/verificationCode/" + _this.phoneData);
					console.log("获取验证码",resp.data);
					
				}catch(e){
					console.log("获取验证码失败",e);
					this.$toast.error("获取验证码失败，请稍后再试");
					setTimeout(function(){
